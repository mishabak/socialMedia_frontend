...
function FriendPost() {
    ...
    var dummy =[
        ...
        post:[
            {data:"https://www.youtube.com/watch?v=ysz5S6PUM-U",type:'video'},
            {data:"priest 1.jpg",type:'image'},
            {data:'Let’s look at some options for iterating over a NodeList, as you get back from running a document',type:'text',backgroundColor:'bg1'}
        ]},
        {id:'d',
        post:[
        ...
        <div>
            {dummy.map((postData,i)=>{
              ...
              <div className={`${friend_post.user_profile_base}`}>
                <div className={`${friend_post.user_profile_base}`}><img className={`${friend_post.profile_img}`} src="Screenshot (334).png" alt="" /> <p className={`${friend_post.userName}`}>User Name</p> 
                </div>
                  ...
                  <Carousel  className={`${friend_post.customSlider} custom-slider`}> 
                   {postData.post.map((content,index)=>{
                       if(content.type=='image'){
                        return <div className={`${friend_post.img_bg}`} > <img  style={{width:imageWidth}} className={`${friend_post.img} ${postData.id} postedImages112`} src={content.data} alt="" />
                        </div>
                       } 
                        if(content.type =='video') {
                        return <div style={{width:imageWidth+'px'}} className={`${friend_post.img_bg}`}  > <ReactPlayer style={{width:'100% !importent'}}  className={`${friend_post.img} ${postData.id} postedImages112`} controls={true}  url={content.data}/> </div>
                       }
                       if(content.type =='text') {
                        return <div className={`${friend_post.img_bg}`} > 
                        <p style={{width:imageWidth ,margin:'initial'}} className={`
                        ${friend_post.img} 
                        ${friend_post.aaa} 
                        ${postData.id} 
                        ...
                        postedImages112` }> <p className={`${friend_post.text_data}`}> {content.data} </p> </p></div>
                       }
